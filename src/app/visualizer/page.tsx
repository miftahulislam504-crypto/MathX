...
export default function VisualizePage() {
  ...
  const TABS: { id: TabId; label: string; icon: string; desc: string; color: string; bg: string }[] = [
    ...
    <>
      ...
      <main className="min-h-screen pt-20 pb-20 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mt-6 grid sm:grid-cols-3 gap-3">
            {[
              { icon: Keyboard, tip: 'Press Enter in any expression box to update the plot instantly.' },
              { icon: Sliders, tip: 'Drag sliders to animate values. Use range inputs for precise control.' },
              { icon: Clipboard, tip: 'Hover formula cards in the Library to copy LaTeX with one click.' },
            ].map(({ icon: Icon, tip }) => (
              <div key={tip} className="flex items-start gap-3 rounded-lg border border-white/5 bg-white/[0.02] p-3">
                <Icon className="w-4 h-4 shrink-0 mt-0.5" />
                <p className="text-xs text-white/30 leading-relaxed">{tip}</p>
