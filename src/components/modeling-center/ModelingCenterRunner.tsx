...
import { getModelingCenterToolBySlug } from '@/lib/data/modelingCenter'

function Spinner({ label }: { label: string }) {
  return (
    <div className="flex items-center justify-center py-20 gap-3">
      <div className="w-6 h-6 border-2 border-white/10 border-t-amber-500 rounded-full animate-spin" />
      <span className="text-white/30 text-sm font-mono">{label}</span>
    </div>
  )
}

export function ModelingCenterRunner({ slug }: { slug: string }) {
    ...
    <main className="min-h-screen pt-24 pb-20 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className={`rounded-xl border ${tool.bg} px-5 py-4 mb-6 flex items-center gap-3`}>
          ...
          <div>
            <p className={`text-base font-semibold ${tool.color}`}>{tt(t.modelingCenter[tool.labelKey])}</p>
            <p className="text-white/40 text-xs mt-0.5">{tt(t.modelingCenter[tool.descKey])}</p>
          </div>
        </div>
