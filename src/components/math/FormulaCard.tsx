...
export function FormulaCard({ formula, compact = false }: Props) {
  ...
  return (
    <div className={cn(
      'group relative rounded-xl border border-white/8 bg-white/[0.03] hover:border-violet-500/30 hover:bg-white/[0.05] transition-all',
      compact ? 'p-4' : 'p-5'
    )}>
      {/* Title row */}
      ...
      {/* Description */}
      <p className={cn('text-white/50 leading-relaxed', compact ? 'text-xs' : 'text-sm')}>
        {formula.description}
      </p>
      ...
      {!compact && (
        <div className="flex flex-wrap gap-1.5 mt-3">
          {formula.tags.map((tag) => (
            <span
              key={tag}
              className="text-[10px] text-violet-400/70 bg-violet-500/8 border border-violet-500/15 rounded-full px-2 py-0.5"
            >
              {tag}
            </span>
          ))}
