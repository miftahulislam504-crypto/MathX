...
export function LatexRenderer({ latex, display = false, className }: Props) {
  ...
  useEffect(() => {
    ...
    import('katex').then((katex) => {
      if (!ref.current) return
      try { katex.default.render(latex, ref.current, { displayMode: display, throwOnError: false, strict: false }) }
      catch { if (ref.current) ref.current.textContent = latex }
    })
  }, [latex, display])
  return <span ref={ref} className={cn(display ? 'block overflow-x-auto py-2 text-center' : 'inline', 'text-white', className)} />
}
