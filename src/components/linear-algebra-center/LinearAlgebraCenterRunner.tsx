...
import { getLinearAlgebraCenterToolBySlug } from '@/lib/data/linearAlgebraCenter'
...
export function LinearAlgebraCenterRunner({ slug }: { slug: string }) {
    ...
    <main className="min-h-screen pt-24 pb-20 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className={`rounded-xl border ${tool.bg} px-5 py-4 mb-6 flex items-center gap-3`}>
          ...
          <div>
            <p className={`text-base font-semibold ${tool.color}`}>{tt(t.linearAlgebraCenter[tool.labelKey])}</p>
            <p className="text-white/40 text-xs mt-0.5">{tt(t.linearAlgebraCenter[tool.descKey])}</p>
          </div>
        </div>
